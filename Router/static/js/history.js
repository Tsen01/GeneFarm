...
function renderTablePage(page) {
    const numericFields = [
        "lstm_prediction_series",
        "bilstm_prediction_series",
        "linear_regression_series",
        "lstm_prediction_last",
        "bilstm_prediction_last",
        "linear_regression_last"
    ];
            ...
            if (Array.isArray(value)) {
                value.forEach(v => {
                    ...
                    if (typeof v === "object" && v !== null) {
                        div.textContent = JSON.stringify(v); // 將物件轉成 JSON 字串
                    } else {
                        const num = parseFloat(v);
                        div.textContent = !isNaN(num) ? num.toFixed(2) : v;
                    }
                    td.appendChild(div);
                });
            } else if (typeof value === "object" && value !== null) {
                td.textContent = JSON.stringify(value); // 物件直接轉 JSON
            } else {
